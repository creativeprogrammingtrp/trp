...
if(is_file($targetDir  . $fileName)){
	...
	$duration = 0;
	if($ext == "mp3"){
		$m = new mp3file($tmp_name);
		$arr_data = $m->get_metadata();
		if(isset($arr_data["Length"]))
			$duration = $arr_data["Length"];
		
		unset ($arr_data);
	}
	$result = array('error'=>'', 'filepath' => 'data/'.$catewrite.'/'.$fileName, 'filename' => $fileName, 'ext' => $ext, 'width'=> $width, 'height'=> $height, 'orgname' => $orgfile, "duration" => $duration);
}else{
	$result = array('error'=> "Could not upload", 'filepath' => '', 'filename' => '', 'ext' => '');
}
