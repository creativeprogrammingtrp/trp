...
class Adjustment extends CI_Controller{
    ...
    function perms(){
        $perms['Bank Adjustment'] = array('index','ShowPenddingApprove','generateFile','download','generateExcel','generateACHFile','generateACHFileCSVFormat','generateACHFileCSVFormatForCustomer');
        return $perms;			
    }
...
<tr><th></th>
        ...
        $data['dataLoadACHCustomer'] = "dataClientACHCustomer = " . json_encode($this->m_clientcenter->loadACHCustomerExportedFileList());
...
//echo $issueDate;
    ...
    function download(){
        ...
        $filepath = './arp_inbound/'.$this->uri->segment(4);
       ...
       // $date = new DateTime();
        ...
        // Have to be start loop here batch wise.
            ...
            foreach ($dipositedApp as $dipApp) {
                ...
                $RCVBNKIDTY = 'ABA';
                $RCVBNKID = $dipApp['bank_routing']; // ERO ROuting number
...
, $RCVBNKIDTY, $RCVBNKID, $ORIGPRTYNM, $ORIGBNKNM,$RCVPRTYNM,$RCVBNKNM,$ACHCMPID);
    ...
    public function generateACHFileCSVFormatForCustomer()
    {
        ...
        // body part
        $dipositedApp = $this->m_clientcenter->loadAllPaidACHApplicationForExportIntoACHByCustomer();
        $totalCheckAmount = 0.00;
        //print_r($printedCheck);
        $k = 0;
        $ecount = 1;
...
/*
            ...
            $totalPayAmount = floatval($txaAmount) + floatval($AddonAmount);
*/
            $totalPayAmount = floatval($dipApp['app_actual_refund_amount']);
            $customername = $dipApp['first_name']." ".$dipApp['last_name'];
            ...
            $RCVBNKIDTY = 'ABA';
            $RCVBNKID = $dipApp['app_routing_no']; // Customer ROuting number
            $ORIGPRTYNM = 'TRP SOLUTION';
            $ORIGBNKNM = 'WELLS FARGO';
            $RCVPRTYNM = $customername; // Customer name
            $RCVBNKNM = $dipApp["app_bank_name"]; //Customer bank name
            $ACHCMPID = '1464683532';
            ...
            $this->m_clientcenter->updateACHExportTImeForAllDepositedCustomerForExport($dipApp['app_id']);
            //  updateACHExportTImeForAllPrintedCheckForExport
            $k++;
            $ecount++;
        ...
        if(sizeof($dipositedApp) > 0){
            ...
            // add exported file name into database for download
            $this->m_clientcenter->addNewFileInfoWithAllCustomerPaymentInfoForExport($filename);
            echo json_encode($this->m_clientcenter->loadACHCustomerExportedFileList());
        }else{
            unlink("./arp_inbound/".$filename);
            echo json_encode("No new record found for generate file. Please try again when record will be available.");
        }
