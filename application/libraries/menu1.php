	...
	function loadMenusSystem(){
		...
		if($query->num_rows() > 0){//0
			foreach($query->result_array() as $row){//1
				...
				$_menus_ .= '<li>';
				$_menus_ .= '<a class="headerNavItem'.$active.'" href="'.$link.'">'.$row['name'].'</a>';
				$_menus_ .= $_menus_child_;	
				$_menus_ .= '</li>';
		...
		if($_menus_ != ''){
			$_menus_ = '<ul class="nav">'.$_menus_.'</ul>';		
		}
	...
	function __loadMenu_child__($id){
		...
		if($query->num_rows() > 0){//0
			foreach($query->result_array() as $row){//1
				...
				$class = 'bg_nobo';
				if($_menus_child_ != '') $class = 'bg_child';
				 
				$str .= '<li><a href="'.$link.'" class="'.$class.'">'.$row['name'].'</a>';
				$str .= $_menus_child_;	
				$str .= '</li>';
			}
		}
		...
		return array('menu_link' => $str, 'active' => $active);
	}
