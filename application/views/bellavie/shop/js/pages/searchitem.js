		...
		if (i<data_clients.length)
		{
			if (type_display == 0)
			{
				switch (i % 3)
				{
					...
					break;
					case 1: str += generate_an_item(data_clients[i]);
					break;
					case 2: str += (generate_an_item(data_clients[i])+'</div>');
					break;
				}
			}
			if (type_display == 1)
...
function generate_an_item(obj)
{
	...
	if (type_display == 0) return '<li class="span3 clearfix"><div class="thumbnail"><a href="'+obj.link+'"><img  src="'+file+'" alt=""></a></div><div class="thumbSetting"><div class="thumbTitle"><a href="'+obj.link+'" class="invarseColor">'+obj.itm_name+'</a>'+obj.outOfStock+'</div><div class="thumbPrice"><span>$'+(obj.itm_price).formatMoney(2, '.', ',')+'</span></div><div class="thumbButtons"><button class="btn btn-primary btn-small" style="margin-right:4px;" data-title="+To Cart" data-placement="top" data-toggle="tooltip"><i class="icon-shopping-cart"></i></button><button class="btn btn-small" data-title="+To WishList" data-placement="top" data-toggle="tooltip" onClick="addToWishlist(\''+obj.itm_key+'\')"><i class="icon-heart"></i></button></div>'+obj.stars+'</div></li>';
	return '<li class="clearfix"><div class="span3"><div class="thumbnail"><a href="'+obj.link+'"><img src="'+file+'" alt=""></a></div></div><div class="span6"><div class="thumbSetting"><div class="thumbTitle"><a href="'+obj.link+'" class="invarseColor">'+obj.itm_name+'</a>'+obj.outOfStock+'</div><div class="thumbPriceRate clearfix"><span>$'+(obj.itm_price).formatMoney(2, '.', ',')+'</span>'+obj.stars+'</div><div class="thumbButtons"><button class="btn btn-primary btn-small" style="margin-right:4px;"><i class="icon-shopping-cart"></i>Add To Cart</button><button class="btn btn-small" style="margin-right:4px;" data-title="+To WishList" data-placement="top" data-toggle="tooltip" data-original-title="" title="" onClick="addToWishlist(\''+obj.itm_key+'\')"><i class="icon-heart"></i></button><button class="btn btn-small" style="margin-right:4px;" data-title="+To Compare" data-placement="top" data-toggle="tooltip" data-original-title="" title=""><i class="icon-refresh"></i></button></div></div></div></li>';
}
...
function get_objects_items(column){
	...
	if ($('#search_des').is(':checked')) keyword_description = keyword;
	ShowLoadingObj({obj:document.getElementById("content_load"), image:"Loader-FloGradient16x16x.gif"});
	$.post("",
	{getdata:"yes",filter_name:keyword_name,filter_description:keyword_description,cat_sub:cat_sub,type:type},
	function(data){
...
function getHash()
{
	...
	cat_sub = getParameter('cat');
	if (cat_sub == null) cat_sub = 0;
	type = getParameter('type');
	if (type == null) type = 0;
	items_per_page = getParameter('pageitem');
	if (items_per_page == null) items_per_page = 6;
	type_display = getParameter('typedisplay');
	if (type_display == null) type_display = 0;
	description = getParameter('des');
...
function set_value_to_input()
{
	...
	$('#appendedInputButton').val(keyword);
	$('#current_cat').val(cat_sub);
	if (type == 0) 
	{
		$('#search_cat').prop('checked', true);
	}
	if (description != 0)
	{
		$('#search_des').prop('checked',true);
	}
...
{
	get_filter_key();
	$('#appendedInputButton').val($('#key_word').val());
	setHash();
	get_objects_items();
}
