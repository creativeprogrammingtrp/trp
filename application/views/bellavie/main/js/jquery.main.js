...
ResponsiveHelper = (function($){
	...
	function resizeHandler() {
		$('.w2').width('');
		$('.tp-bannershadow.tp-shadow1').width('');
		if (parseInt($('.w2').width()) > parseInt($(window).width()))
		{
			$('.w2').width($(window).width());
			$('.tp-bannershadow.tp-shadow1').width($(window).width());
		}
		var scroll_width = 0;
		try
		{
			scroll_width = scrollSize.getWidth();
		}
	  catch(err)
		{
			scroll_width = 0;
		}
		var winWidth = win.width() + scroll_width;
			...
			if(inputs[i].className.indexOf(opt.skipClass) < 0) {
				...
				if((opt.clearInputs && (inputType === 'text' || inputType === 'email')) ||
					...
					new PlaceholderInput({
						...
						placeholderAttr: inputs[i].getAttribute('nplaceholder') ? 'nplaceholder' : opt.placeholderAttr
					});
				}
			}
		}
...
jcf.addModule({
	...
	setupWrapper: function(){
		...
		if (this.realElement.id == "menu-select") this.fakeElement.style.width = '300px';
		else
