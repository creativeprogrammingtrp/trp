...
class Mpermissions extends CI_Model {
	...
	function truncateRolesAccess(){
		$this->db->truncate('roles_permission'); 	
	}
	...
	function updateUsersAccess(){
		if(count($this->dataPost) > 0){//0
			foreach($this->dataPost as $data){//1
				$query = $this->db->query("select users_permission.*,users_roles.rid from users_permission join users_roles on users_roles.uid =  users_permission.uid where users_roles.rid = ".$data['rid']);
				if ($query->num_rows() > 0){
					foreach ($query->result_array() as $row){
